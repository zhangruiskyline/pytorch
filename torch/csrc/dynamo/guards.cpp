...
class DICT_VERSION : public LeafGuard {
 public:
  DICT_VERSION(py::object value, py::object guard_str) : LeafGuard(guard_str) {
...
class GuardManager {
 public:
  ...
  virtual GuardManager* get_key_manager() {
    throw std::runtime_error("get_key_manager is not implemented");
  }

  virtual GuardManager* get_value_manager() {
...
class KeyValueDictGuardManager : public GuardManager {
 public:
  KeyValueDictGuardManager(RootGuardManager* root, py::handle example_value)
      : GuardManager(root) {
    py::tuple example_value_tuple = py::cast<py::tuple>(example_value);
    _key_manager = make_guard_manager(root, example_value_tuple[0]);
    _value_manager = make_guard_manager(root, example_value_tuple[1]);
  }

  virtual GuardManager* get_key_manager() override {
    return _key_manager.get();
  }

  virtual GuardManager* get_value_manager() override {
    return _value_manager.get();
  }

  virtual bool check_nopybind(PyObject* item) override { // borrowed ref
    // We get the key, value pair from the DictGuardManager here. Check the key
    // guard manager and then value guard manager. There is no need to do any
    // shuffling here.
    ...
    bool result = _key_manager->check_nopybind(key);
    if (!result) {
      _fail_count += 1;
      return result;
    }
  ...
  virtual GuardDebugInfo check_verbose_nopybind(
    ...
    // We get the key, value pair from the DictGuardManager here. Check the key
    // guard manager and then value guard manager.
    ...
    GuardDebugInfo key_debug_info = _key_manager->check_verbose_nopybind(key);
    if (!key_debug_info.result) {
      return key_debug_info;
    }

    int num_guards_executed = key_debug_info.num_guards_executed;
    GuardDebugInfo value_debug_info =
        _value_manager->check_verbose_nopybind(value);
    return GuardDebugInfo(
        value_debug_info.result,
        value_debug_info.failed_guard,
        num_guards_executed + value_debug_info.num_guards_executed);
  };

  std::string repr() const {
    return "KeyValueDictGuardManager";
  }
...
class DictGuardManager : public GuardManager {
 public:
  ...
  virtual GuardManager* get_key_value_manager(
      const py::object& accessor_key,
      py::handle example_value) override {
    ...
    _key_value_managers[index] = std::make_unique<KeyValueDictGuardManager>(
        this->get_root(), example_value);
  ...
  py::class_<GuardManager, std::unique_ptr<GuardManager>>(py_m, "GuardManager")
      ...
      .def(
          ...
          [](GuardManager& self,
            ...
            return self.get_key_value_manager(index, example_value);
          },
          py::return_value_policy::reference)
      .def(
          "get_key_manager",
