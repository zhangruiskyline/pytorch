...
def remove_cat(match: Match, split_sections: List[int], dim: int):
    ...
    for cat_user in next_users:
        if cat_user.target == torch.cat:
            ...
            if len(split_sections) == len(cat_user.args[0]):
                ...
                if not is_node_meta_valid(split_node.args[0]):
                    continue
                ...
                slice_list = []
                for i in range(len(split_node.args[0].meta["example_value"].shape)):
                    if i != split_dim:
                ...
                with graph.inserting_after(split_node):
                    ...
                    slice_node.meta.update(split_node.args[0].meta)
