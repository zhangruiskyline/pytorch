...
class _DerivedDim(_Dim):
    ...
    Currently we only support monotonic linear expressions (with integer coefficients).
    Such an operation is expressed by a function `fn` applied over a normal Dim, `root`.
    The range for a derived Dim is computed by mapping `fn` over the range of its `root`.
    """

    @property
    def min(self):
        # assume monotonic
        # TODO(avik): use sympy value range analysis
    ...
    def _derive(self, name, fn):
        ...
        # (implemented by composing operations on the same root)
        return _DerivedDim(
    ...
    if isinstance(constraint, _DerivedConstraint):
        if isinstance(constraint.root, _PhantomRoot):
            if constraint.root.name in phantom_symbols:
                ...
                # create a symbol in the shape env whenever `constraint` has a phantom root
        ...
        fn = constraint.fn
        # derived equality (source, root, fn) informally corresponds to source = fn(root)
